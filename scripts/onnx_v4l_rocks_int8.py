...
class RocksV4LDataReader(CalibrationDataReader):
    def __init__(self, calib_path):
        # Load NCHW [100, 3, 244, 244] - ONNX V4L expects NCHW
...
def quantize_v4l_rocks_onnx_int8(model_path, calib_path, output_path):
    print(f"Quantizing {model_path} to ONNX INT8...")
    dr = RocksV4LDataReader(calib_path)
    quantize_static(
        model_path,
        output_path,
        dr,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lithotheque V4L Rocks ONNX INT8 Quantizer")
    ...
    parser.add_argument("--out", type=str, default="roches_v4l_int8.onnx", help="Output path")
    args = parser.parse_args()
    
    if os.path.exists(args.model):
        quantize_v4l_rocks_onnx_int8(args.model, args.calib, args.out)
    else:
