...
class RocksV5DataReader(CalibrationDataReader):
    def __init__(self, calib_path):
        # Load NCHW and transpose to NHWC for ONNX V5
...
def quantize_v5_rocks_onnx_int4(model_path, calib_path, output_path):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lithotheque V5 Rocks ONNX INT4 Quantizer")
    ...
    parser.add_argument("--out", type=str, default="roches_v5_int4.onnx", help="Output path")
    args = parser.parse_args()
    
    if os.path.exists(args.model):
        quantize_v5_rocks_onnx_int4(args.model, args.calib, args.out)
    else:
